...
export default function ConfirmationTracker({ employees }: Props) {
  ...
  const activeRows = useMemo(() => {
    switch (activeGroup) {
      ...
      case 'dueWeek': return dueThisWeek;
      default: return [];
    }
  }, [activeGroup, confirmed, pending, overdue, dueThisWeek]);
  const activeTitle = {
    confirmed: 'Confirmed Employees',
    pending: 'Pending Confirmations',
    overdue: 'Overdue Confirmations',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <button onClick={() => setActiveGroup('overdue')} className="text-left">
        ...
        <button onClick={() => setActiveGroup('dueWeek')} className="text-left">
    ...
    <div className="fixed inset-0 z-50 bg-black/40 p-4 flex items-center justify-center">
      <div className="w-full max-w-6xl rounded-2xl bg-white shadow-2xl max-h-[88vh] overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
          ...
          <button onClick={onClose} className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">
            Close
          </button>
        </div>
